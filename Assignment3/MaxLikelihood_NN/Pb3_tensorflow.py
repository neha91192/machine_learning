...
class Tensorflow:
    ...
    def train(self, X, Y):
        ...
        layer_1_output = tf.nn.relu(tf.matmul(X, weights1) + biases1)
        ...
        output = tf.matmul(layer_1_output, weights2) + biases2
        prediction = tf.nn.softmax_cross_entropy_with_logits_v2(labels=Y, logits=output)
